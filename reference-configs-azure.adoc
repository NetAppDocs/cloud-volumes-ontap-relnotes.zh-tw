...
. ^6^ Edsv6 VM 類型支援Cloud Volumes ONTAP 9.17.1 及更高版本的全新部署。您無法將現有部署切換為任何其他 VM 類型，例如從 Edsv5 切換到 Edsv6；僅支援 Edsv6 變體之間的大小變更（例如 E20ds_v6 → E32ds_v6）。有關此虛擬機器類型的信息，請參閱 https://learn.microsoft.com/en-us/azure/virtual-machines/sizes/memory-optimized/edsv6-series?tabs=sizebasic["Azure 文件：Edsv6 尺寸系列"^]。
...
. ^8^ Edsv6 VM 類型支援Cloud Volumes ONTAP 9.17.1 及更高版本的全新部署。您無法將現有部署切換為任何其他 VM 類型，例如從 Edsv5 切換到 Edsv6；僅支援 Edsv6 變體之間的大小變更（例如 E20ds_v6 → E32ds_v6）。有關此虛擬機器類型的信息，請參閱 https://learn.microsoft.com/en-us/azure/virtual-machines/sizes/memory-optimized/edsv6-series?tabs=sizebasic["Azure 文件：Edsv6 尺寸系列"^]。
...
| 支援的磁碟類型 3+| 進階 SSD 託管磁碟或進階 SSD v2 託管磁碟。 
|===
附註：
